...
pub(crate) async fn deliver_start_failures(
    ...
    for notice in start_notice::pending(db)? {
        ...
        if let Err(error) = result {
            eprintln!(
                "reserve_start_failure_notice_deferred job_id={} error={error}",
                notice.job_id
            );
...
pub(crate) async fn deliver_reserve_transition_notices(
    db: &std::path::Path,
    http: &twilight_http::Client,
) -> Result<(), CompletionWorkerError> {
    let mut first_error = None;
    for notice in cdr_store::reserve_policy::transition_notice::pending(db)? {
        let result = async {
            let Some(channel_id) = notice.channel_id else {
                return Err(CompletionWorkerError::Delivery(
                    "Reserve transition notice has no current Discord mapping".into(),
                ));
            };
            super::receipt::send_chunk(
                db,
                http,
                i64_channel(channel_id)?,
                &IdempotentChunk {
                    domain: cdr_store::reserve_policy::transition_notice::DOMAIN,
                    logical_key: notice.notice_id.clone(),
                    chunk_index: 0,
                    content: notice.content.clone(),
                },
            )
            .await?;
            cdr_store::reserve_policy::transition_notice::complete(db, &notice.notice_id)?;
            Ok::<(), CompletionWorkerError>(())
        }
        .await;
        if let Err(error) = result {
            eprintln!(
                "reserve_transition_notice_deferred notice_id={} thread_id={} error={error}",
                notice.notice_id, notice.thread_id
            );
            first_error.get_or_insert(error);
        }
    }
