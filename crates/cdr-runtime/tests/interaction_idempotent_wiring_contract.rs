...
fn interaction_create_message_paths_use_stable_idempotent_delivery() {
    ...
    assert!(INTERACTION_WORKER.contains("deliver_interaction_text_idempotent("));
    assert!(INTERACTION_WORKER.contains("send_recorded_message_with_components("));
    assert!(INTERACTION_WORKER.contains("&work.custody_database"));
    assert!(INTERACTION_WORKER.contains("InteractionErrorDisposition::IgnoreDuplicate"));
