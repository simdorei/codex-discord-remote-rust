...
impl PidCleanup {
    ...
    pub fn diagnostic(&self) -> String {
        format!("pid-file={}", describe_file(&self.path, 128))
    }
...
pub async fn settle_failure(
    ...
    if let Ok(result) = tokio::time::timeout(Duration::from_secs(8), &mut *task).await {
        format!("operation settlement={result:?}")
    } else {
        task.abort();
...
#[test]
fn pid_helper_timeout_terminates_the_retained_helper() {
